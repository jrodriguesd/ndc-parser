  ...
  this.parseDataCommand = function(splitted){
    ...
    case '1':
      ...
      case '6':
        ...
        break;
      case 'A':
        parsed.message_identifier = 'Enhanced Configuration load';
        parsed.LUNO = splitted[4];
        break;
      case 'E':
        parsed.message_identifier = 'Currency Cassette Mapping Table';
        parsed.number_of_entries = splitted[4].substr(0, 2);
		let pos = 2;
        parsed.cassettes = [];
        for(let i = 1; i < (parseInt(parsed.number_of_entries) + 1); i++)
		{
		  let obj = '';    
          obj += splitted[4].substr(pos, 2); pos += 2;
          obj += splitted[4].substr(pos, 1); pos += 1;
          obj += splitted[4].substr(pos, 5); pos += 5;
          parsed.cassettes.push(obj);
        }
        break;
      default:
        break;
    ...
    case '4':
      ...
      case '2':
		...
		if ( (splitted[4].substr(0, 3) === '018') &&
		     (splitted[4].substr(3).length == 24) )
		{
		  parsed.new_key_length = splitted[4].substr(0, 3);
          parsed.new_key_data = splitted[4].substr(3);
		}
		
		if ( (splitted[4].substr(0, 3) === '030') &&
		     (splitted[4].substr(3).length == 48) )
		{
		  parsed.new_key_length = splitted[4].substr(0, 3);
          parsed.new_key_data = splitted[4].substr(3);
		}
